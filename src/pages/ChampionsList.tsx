...
import championTierP from "../assets/icon-champion-p.png";
...
export default class ChampionsList extends React.Component<ChampionListProps, ChampionListState> {
    ...
    onClickTrendPosition = (position: string) => async () => {
        const trendChampions = await this.getTrendList(this.state.type, position);
        this.setState({
            trendChampions, 
    ...
    ChangeTrendHead = (type: string) => {
        const head = [['#'],['챔피언']];
        switch(type) {
            case 'tier':
                return head.concat(['승률'],['픽률'],['티어']);
            case 'winratio':
                return head.concat(['승률'],['픽률']);
            case 'pickratio':
                return head.concat(['픽률'],['승률']);
            case 'banratio':
                return head.concat(['밴률']);
            default:
                return [];
        }
            ...
            <ChampionListPageWrapper>
                ...
                <ChampionTrendWrapper>
                    <div className="header">
                        <div>챔피언 순위</div>
                        <div className="item-wrap">
                            <div className={classnames("item", {select: this.state.trendType === "tier"})} onClick={this.onClickTrendType("tier")}><img src={this.state.trendType === "tier" ? championTierP : championTierN}/>티어</div>
                    ...
                    <div className="list">
                        <ChampionTrendToolbar>
                            <div hidden={this.state.trendType === "tier"} className={classnames({select: this.state.trendPosition === "all"})} onClick={this.onClickTrendPosition("all")}>전체</div>
                            <div className={classnames({select: this.state.trendPosition === "top"})} onClick={this.onClickTrendPosition("top")}>탑</div>
                            <div className={classnames({select: this.state.trendPosition === "jug"})} onClick={this.onClickTrendPosition("jug")}>정글</div>
                            ...
                            <div className={classnames({select: this.state.trendPosition === "sup"})} onClick={this.onClickTrendPosition("sup")}>서폿</div>
                        </ChampionTrendToolbar>
                        <ChampionTrendHeader>
                            {
                                this.ChangeTrendHead(this.state.trendType).map(t => {
                                    return(<div>{t}</div>)
                                })
                            }
